...
final class MakerTestEnvironment
{
    ...
    {
        if (!file_exists(__DIR__.'/../../tests/fixtures/php-cs-fixer/vendor/bin/php-cs-fixer')) {
            throw new \Exception('php-cs-fixer not found: run: "composer install --working-dir=tests/fixtures/php-cs-fixer".');
        }

        return MakerTestProcess::create(
            sprintf('php tests/fixtures/php-cs-fixer/vendor/bin/php-cs-fixer --config=%s fix --dry-run --diff %s', __DIR__.'/../Resources/test/.php_cs.test', $this->path.'/'.$file),
            $this->rootPath,
    ...
    private function buildFlexSkeleton(): void
    {
        ...
        MakerTestProcess::create('composer require symfony/psr-http-message-bridge \
                woohoolabs/yin:^4.3 symfony/validator symfony/orm-pack \
                phpunit doctrine/doctrine-bundle \
                ...
                symfony/translation \
                nyholm/psr7 \
                --prefer-dist --no-progress --no-suggest', $this->flexPath)
            ->run();
